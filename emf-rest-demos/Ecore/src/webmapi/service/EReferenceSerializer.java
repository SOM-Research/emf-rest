...
public final class EReferenceSerializer { 
 public static String serializeEReferenceList(java.util.List<org.eclipse.emf.ecore.EReference> obj, int depth, OutputStream out) throws RuntimeException {
        ByteArrayOutputStream bStream = new ByteArrayOutputStream();
        PrintWriter pw = new PrintWriter(out == null ? new ByteArrayOutputStream() : out);
        JSONWriter jw = new JSONWriter(pw);
        jw.array();
        for (org.eclipse.emf.ecore.EReference oi : obj) {
            generateJSONObject(jw, (org.eclipse.emf.ecore.EReference)oi, new HashMap<String, String>(), new StringBuffer(), depth);
        }
        jw.endArray();
        if (out == null) {
            try {
	...
	public final static String serialize(org.eclipse.emf.ecore.EReference obj, OutputStream out) throws RuntimeException {
		return serialize(obj, 0, out);
	}

	public final static String serialize(org.eclipse.emf.ecore.EReference obj, int depth, OutputStream out) {
		ByteArrayOutputStream bStream = new ByteArrayOutputStream();
...
eTypeEClass  
 ...
 , eTypeEEnum  
	    ...
	    jw.key("eReferenceType");
			...
			// else
			if (serialized.get("EClass"+IdentificationResolver.getEClassId(obj.getEReferenceType())) == null) {
		     	webmapi.service.EClassSerializer.generateJSONObject(jw, obj.getEReferenceType(), serialized, path, depth - 1);
			}else{
			    jw.value(IdentificationResolver.getEClassId(obj.getEReferenceType()));
			}
