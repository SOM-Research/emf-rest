...
public final class EModelElementContainerSerializer { 
 public static String serializeEModelElementContainerList(java.util.List obj, int depth, OutputStream out) throws RuntimeException {
        ByteArrayOutputStream bStream = new ByteArrayOutputStream();
        PrintWriter pw = new PrintWriter(out == null ? new ByteArrayOutputStream() : out);
        JSONWriter jw = new JSONWriter(pw);
        jw.array();
        for (Object oi : obj) {
            generateJSONObject(jw, ( webmapi.service.container.EModelElementContainer)oi, new HashMap<String, String>(), new StringBuffer(), depth);
        }
        jw.endArray();
        if (out == null) {
            try {
	...
	public final static String serialize( webmapi.service.container.EModelElementContainer obj, OutputStream out) throws RuntimeException {
		return serialize(obj, 0, out);
	}

	public final static String serialize( webmapi.service.container.EModelElementContainer obj, int depth, OutputStream out) {
		ByteArrayOutputStream bStream = new ByteArrayOutputStream();
   ...
   public final static void generateJSONObject(JSONWriter jw, webmapi.service.container.EModelElementContainer obj, HashMap<String, String> serialized, StringBuffer path, int depth) {
     jw.object();
		if(obj == null){
		  jw.endObject();
		  return;
     ...
     jw.key("EAnnotation"); 
	 jw.array();
	 for (org.eclipse.emf.ecore.EAnnotation obji : obj.getEAnnotation()) {
     ...
     jw.key("EFactory"); 
	 jw.array();
	 for (org.eclipse.emf.ecore.EFactory obji : obj.getEFactory()) {
		if (serialized.get("EFactory"+IdentificationResolver.getEFactoryId(obji)) == null) {
			webmapi.service.EFactorySerializer.generateJSONObject(jw, obji, serialized, path, depth - 1);
		}else{
			// put a reference???
		}
	 }
	 jw.endArray();	 
     jw.key("ENamedElement"); 
	 jw.array();
	 for (org.eclipse.emf.ecore.ENamedElement obji : obj.getENamedElement()) {
		if (serialized.get("ENamedElement"+IdentificationResolver.getENamedElementId(obji)) == null) {
			webmapi.service.ENamedElementSerializer.generateJSONObject(jw, obji, serialized, path, depth - 1);
		}else{
			// put a reference???
		}
	 }
