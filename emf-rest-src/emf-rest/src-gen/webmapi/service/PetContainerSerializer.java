...
public final class PetContainerSerializer { 
 public static String serializePetContainerList(java.util.List obj, int depth, OutputStream out) throws RuntimeException {
        ByteArrayOutputStream bStream = new ByteArrayOutputStream();
        PrintWriter pw = new PrintWriter(out == null ? new ByteArrayOutputStream() : out);
        JSONWriter jw = new JSONWriter(pw);
        jw.array();
        for (Object oi : obj) {
            generateJSONObject(jw, ( webmapi.service.container.PetContainer)oi, new HashMap<String, String>(), new StringBuffer(), depth);
        }
        jw.endArray();
        if (out == null) {
            try {
	...
	public final static String serialize( webmapi.service.container.PetContainer obj, OutputStream out) throws RuntimeException {
		return serialize(obj, 0, out);
	}

	public final static String serialize( webmapi.service.container.PetContainer obj, int depth, OutputStream out) {
		ByteArrayOutputStream bStream = new ByteArrayOutputStream();
   ...
   public final static void generateJSONObject(JSONWriter jw, webmapi.service.container.PetContainer obj, HashMap<String, String> serialized, StringBuffer path, int depth) {
     jw.object();
		if(obj == null){
		  jw.endObject();
		  return;
     ...
     jw.key("Cat"); 
	 jw.array();
	 for (Example.Cat obji : obj.getCat()) {
		if (serialized.get("Cat"+IdentificationResolver.getCatId(obji)) == null) {
			webmapi.service.CatSerializer.generateJSONObject(jw, obji, serialized, path, depth - 1);
		}else{
		}
	 }
	 jw.endArray();	 
